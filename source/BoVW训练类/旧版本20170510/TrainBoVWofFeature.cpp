...
//构建BoVW特征采集函数与BoVW计算
bool TrainBoVWofFeature::getSamplesOfFeature(int data_type)
{
	...
	if(data_type==0)//选择摄像头
	{
		//初始化摄像头
		cout<<"摄像头初始化..."<<endl;
		capture=cv::VideoCapture(0);//存储摄像头拍摄的数据指针
		if(!capture.isOpened()) // 打开摄像头
		{
			MessageBox(0, "无法打开摄像头","error", MB_ICONERROR);
			return -1;
		}
	}
	if(data_type==1)//选择图片集
	{
		...
		if(!capture.isOpened()) // 打开图片路径
		{
			...
			if(!capture.isOpened()) // 打开图片路径
			{
				MessageBox(0, "无法打开指定文件","error", MB_ICONERROR);
				return -1;
			}
		}
	}
	...
	//选择是否继续上一次的采样
	int result=MessageBox(0, "是否继续上一次的采样？","选择", MB_YESNO);
	...
	while(1)
	{
		...
		cout<<"当前总的特征数目为"<<samples_of_feature_.size()+lines_of_file<<endl;
		if(cv::waitKey(50)>0||samples_of_feature_.size()+lines_of_file>1000000)//特征数目过多，强制退出
		{
			break;
		}
	}
...
bool TrainBoVWofFeature::buildBagofVisvalWord()
{
	...
	int result=MessageBox(0, "是否开始训练视觉词典","选择", MB_YESNO);
	...
	while(!infile.eof())//一直读到文件末尾
	{
		...
		input_data.resize(space_position_sz-1);

		input_data[0]=atof(file_data.substr(0,space_position[0]).c_str());	//特征数据

		for(int i=1;i<space_position_sz-1;i++)
		{
	...
	for(int i=0;i<tree_levels;i++)
	{
		...
		if(i==0)//如果是第1层
		{
			...
			for(int j=0;j<samples_belones_result.size();j++)
			{
		...
		for(int j=0;j<samples_belones_upper_level.size();j++)
		{
	...
	result=MessageBox(0, "是否储存本次 BoVW 训练结果?","选择", MB_YESNO);
	if(result==IDNO)//退出buildBagofVisvalWord
	{
...
bool TrainBoVWofFeature::getIndexofFeature(vector<float>& input_feature_data,vector<int>& feature_index)
{
	...
	{
		cout<<"error:输入特征长度正确。正确长度为："<<bag_of_visual_word_.kd_tree_data[0].size()<<endl;
		return false;
	}
